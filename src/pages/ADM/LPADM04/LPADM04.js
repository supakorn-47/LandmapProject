...
export default function LPADM04() {
  ...
  const onGetDataSurveyUserByFormID = (rowData) => {
    ADM07GetDataSurveyUserByFormID(rowData).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
          setDialog({
  ...
  const onCreateDataSurveyUser = (list) => {
    setLoading(true);
    ADM07CreateDataSurveyUser(dialog.data, list).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
  ...
  const onGetDataSurveyListByFormID = (rowData) => {
    setLoading(true);
    ADM07GetDataSurveyListByFormID(rowData).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
          setDialog({
      ...
      function (err) {
        ...
        showMessages(
          "error",
          `เกิดข้อผิดพลาด Status Code: ${err.response.data.status}`,
    ...
    if (dialog.action === "เพิ่ม" && validation(data)) {
      ADM07CreateData(data).then(
        ...
        function (err) {
          ...
          showMessages(
            "error",
            `เกิดข้อผิดพลาด ${err.response.data.errors.message}`,
            ""
          );
        }
      );
    } else if (dialog.action === "แก้ไข" && validation(data)) {
      ADM07UpdateData(data).then(
        (res) => {
          setLoading(false);
          if (res.status === 200) {
      ...
      setLoading(true);
      ADM07AddDataSurveyList(data).then(
        (res) => {
          setLoading(false);
          if (res.status === 200) {
  ...
  const footerButton = () => {
    const onADM07DeleteData = () => {
      ADM07DeleteData(deleteDialog.data.form_seq).then(
        (res) => {
          setLoading(false);
          if (res.status === 200) {
  ...
  return (
    <div className="page-wrapper">
      ...
      {dialog.dialogConfigQuestion === true ||
      dialog.dialog === true ||
      dialog.dialogGroup === true ? (
        <LPADM04Dialog
          dialog={dialog}
          setDialog={setDialog}
