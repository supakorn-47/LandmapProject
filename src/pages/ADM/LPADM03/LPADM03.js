...
import LPADM03Dialog from "./LPADM03Dialog";
...
export default function LPADM03() {
  ...
  useEffect(() => {
    ...
    masterService(`GetAnnounceType?mode=1`, {}, "GET").then(
      (res) => {
      ...
      function (err) {
        ...
        showMessages(
          "error",
          `เกิดข้อผิดพลาด Status Code: ${err.response.status} ${err.response.message}`,
          ""
        );
      }
    );
  ...
  const onGetDataList = () => {
    setLoading(true);
    ADM06Services.GetDataList(searchData).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
          let temp = [];
  ...
  const onGetAnnounceFileList = (rowData, check = "") => {
    setLoading(true);
    ADM06Services.GetAnnounceFileList(rowData).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
          if (check === "") {
  ...
  const validation = (object) => {
    if (
      ...
      object.announce_title_th === "" ||
      object.announce_title_en === "" ||
      object.announce_desc_th === "" ||
      object.announce_desc_en === "" ||
      object.service_url === "" ||
      object.announce_type === -1 ||
      object.announce_type === "-1"
    ) {
      setSubmitted(true);
      return false;
  ...
  const submitForm = (submitForm, arrFile, arrImg) => {
    ...
    for (let index = 0; index < files.length; index++) {
      if (
        files[index].announce_file_seq === undefined &&
        files[index].file !== ""
      ) {
